...
void register_peripheral(const char *name, uint32_t base_addr, uint32_t len,
...
bool is_peripheral(uint32_t addr, size_t *id) {
  ...
  for (int i = 0; i < num_peripherals; i++) {
    if (addr >= peripherals[i].base_addr &&
        addr <= peripherals[i].base_addr + peripherals[i].len) {
      *id = i;
      return true;
    }
  }
...
void init_cpu(const char *fname) {
  ...
  register_peripheral("Digit", 0xFFFFF000, 0x4, read_seven_seg,
                      write_seven_seg);
  init_memory(fname);
}
