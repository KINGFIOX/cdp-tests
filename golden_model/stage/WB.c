...
WB_info WB(MEM2WB mem_info)
{
  ...
  {
    // 访问全局状态
    // 如果前面有发生 branch/jal/jalr 的话, npc 就会被更新, 否则默认是 npc = pc + 4
  ...
  if (mem_info.wb_en)
  {
    ...
    {
    case WB_ALU:
      wb_val = mem_info.alu_out;
      break;
    case WB_PC:
      wb_val = mem_info.pc + 4;
      break;
    case WB_LOAD:
      wb_val = mem_info.load_out;
      break;
    default:
      wb_val = 0;
      break;
  ...
  ret.wb_value = wb_val; // 确实没有完全初始化, 但是好像没啥关系
  ...
  if (mem_info.wb_en)
  {
    Log("WB value = %8.8x, WReg = %d, npc = 0x%8.8x", wb_val, mem_info.dst,
        cpu.npc);
  }
  if (mem_info.branch_taken)
  {
